 ...
 *
 */
void add_hashnode(char *mood, double *scale)
{
 ...
 */
double *get_random_hashkey(char *mood)
{
   hashmap_node_t *current = mood_notes->head;
   int mood_size = strlen(mood); //strlen is expensive so idk if we want to call it or not
...
/**
 * This will make a num_notes chord based on the scale and degree you give it. 
 * THIS WAS BASED ON THE CHORD KNOWLEDGE BELOW AND THE BITS I KNOW ABOUT IT
 * 
...
double* make_chord(double * scale, int chord_deg, int num_notes){
   ...
   //returns the chord that you want
   return chord;
}

/**
 * This function will populate the hashmap based on the provided arrays after all keys
*/
